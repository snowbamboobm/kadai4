...
echo "2" > $tmp-ans
diff $tmp-reselt $tmp-ans && echo "OK" || ERROR_EXIT "error in test6-1"
rm $tmp-result $tmp-ans

#test6-2：引数が正常
./gcd.sh 12 18> $tmp-result
echo "6" > $tmp-ans
diff $tmp-reselt $tmp-ans && echo "OK" || ERROR_EXIT "error in test6-2"
rm $tmp-result $tmp-ans

#test6-3：引数が正常
./gcd.sh 7 11> $tmp-result
echo "1" > $tmp-ans
diff $tmp-reselt $tmp-ans && echo "OK" || ERROR_EXIT "error in test6-3"
rm $tmp-result $tmp-ans

#test6-4：引数が正常
./gcd.sh 100 90> $tmp-result
echo "10" > $tmp-ans
diff $tmp-reselt $tmp-ans && echo "OK" || ERROR_EXIT "error in test6-4"
rm $tmp-result $tmp-ans

#test6-5：引数が正常
./gcd.sh 50 75> $tmp-result
echo "25" > $tmp-ans
diff $tmp-reselt $tmp-ans && echo "OK" || ERROR_EXIT "error in test6-5"
rm $tmp-result $tmp-ans
